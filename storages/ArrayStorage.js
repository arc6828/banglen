...
// const STORAGE_KEY = "@incomes"; //เปลี่ยนแค่ KEY 
//ดึงข้อมูลทั้งหมดขึ้นมาแสดง
const readItems = async (STORAGE_KEY) => {
  try {
...
const readItemDetail = async (STORAGE_KEY,item) => {
  try {
    let string_value = await AsyncStorage.getItem(STORAGE_KEY);
    let items = string_value != null ? JSON.parse(string_value) : [];
...
const removeItem = async (STORAGE_KEY,item) => {
  try {
    //READ ALL
    let string_value = await AsyncStorage.getItem(STORAGE_KEY);
    let items = string_value != null ? JSON.parse(string_value) : [];
...
const writeItem = async (STORAGE_KEY,item) => {
  // console.log("PRESS!!");
  try {
    //READ ALL
    let string_value = await AsyncStorage.getItem(STORAGE_KEY);
